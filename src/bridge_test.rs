...
#[test]
fn test_bridge_wrap_in_threshold_not_met_fails() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    let sig1 = sign_inbound_payload(&env, &signing_key1, &client.address, source_chain, source_nonce, &recipient, period, &archetype, &data_hash);
    let mut signatures = soroban_sdk::Vec::new(&env);
    signatures.push_back(sig1); // only 1 signature
...
#[test]
fn test_bridge_wrap_in_duplicate_signer_fails() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    signatures.push_back(sig1); // Duplicate signature
