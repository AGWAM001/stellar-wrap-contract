...
/// Signs the canonical mint payload that the contract rebuilds in `mint::mint_wrap`.
fn sign_payload(
    env: &Env,
    signer: &SigningKey,
    contract: &Address,
...
fn test_duplicate_period_fails() {
    ...
    let sig = sign_payload(&env, &signing_key, &contract_id, &user, period, &archetype, &hash, CURRENT_PAYLOAD_VERSION);
...
#[test]
fn test_valid_period_boundary_succeeds() {
    let env = Env::default();
    let contract_id = env.register_contract(None, StellarWrapContract);
    let client = StellarWrapContractClient::new(&env, &contract_id);
    ...
    let lower_hash = BytesN::from_array(&env, &[1u8; 32]);
    let upper_hash = BytesN::from_array(&env, &[2u8; 32]);
    let archetype = symbol_short!("arch");

    let lower_sig = sign_payload(&env, &signing_key, &contract_id, &user, 202401, &archetype, &lower_hash, CURRENT_PAYLOAD_VERSION);
    let upper_sig = sign_payload(&env, &signing_key, &contract_id, &user, 210012, &archetype, &upper_hash, CURRENT_PAYLOAD_VERSION);
...
fn test_mint_wrap_zero_hash_rejected() {
    ...
    let sig = sign_payload(&env, &signing_key, &contract_id, &user, period, &archetype, &zero_hash, CURRENT_PAYLOAD_VERSION);
...
fn test_mint_wrap_non_zero_hash_succeeds() {
    ...
    let sig = sign_payload(&env, &signing_key, &contract_id, &user, period, &archetype, &edge_hash, CURRENT_PAYLOAD_VERSION);
...
fn test_mint_wrap_max_hash_succeeds() {
    ...
    let sig = sign_payload(&env, &signing_key, &contract_id, &user, period, &archetype, &max_hash, CURRENT_PAYLOAD_VERSION);
...
fn test_balance_of_and_count() {
    ...
    let sig2 = sign_payload(&env, &signing_key, &contract_id, &user, 202402, &archetype, &hash2, CURRENT_PAYLOAD_VERSION);
...
fn test_total_wrap_count_tracks_mints_across_users() {
    ...
    let sig_a = sign_payload(&env, &signing_key, &contract_id, &user_a, 202401, &archetype, &hash, CURRENT_PAYLOAD_VERSION);
    client.mint_wrap(&user_a, &202401, &archetype, &hash, &CURRENT_PAYLOAD_VERSION, &sig_a);
    assert_eq!(client.total_wrap_count(), 1);

    let sig_a2 = sign_payload(&env, &signing_key, &contract_id, &user_a, 202402, &archetype, &hash, CURRENT_PAYLOAD_VERSION);
    client.mint_wrap(&user_a, &202402, &archetype, &hash, &CURRENT_PAYLOAD_VERSION, &sig_a2);
    assert_eq!(client.total_wrap_count(), 2);

    let sig_b = sign_payload(&env, &signing_key, &contract_id, &user_b, 202401, &archetype, &hash, CURRENT_PAYLOAD_VERSION);
...
fn test_verify_data_matching_hash() {
    ...
    let sig = sign_payload(&env, &signing_key, &contract_id, &user, period, &archetype, &data_hash, CURRENT_PAYLOAD_VERSION);
...
fn test_get_latest_wrap_returns_most_recent() {
    ...
    let sig1 = sign_payload(&env, &signing_key, &contract_id, &user, 202402, &archetype, &hash1, CURRENT_PAYLOAD_VERSION);
    let sig2 = sign_payload(&env, &signing_key, &contract_id, &user, 202404, &archetype, &hash2, CURRENT_PAYLOAD_VERSION);
    let sig3 = sign_payload(&env, &signing_key, &contract_id, &user, 202403, &archetype, &hash3, CURRENT_PAYLOAD_VERSION);
...
fn test_old_wrap_preserved_on_new_mint() {
    ...
    let wrap2 = client.get_wrap(&user, &202402).unwrap();
