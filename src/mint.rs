...
pub(crate) fn mint_wrap(
    e: Env, user: Address, period: u64, archetype: Symbol,
    data_hash: BytesN<32>, payload_version: u32, signature: BytesN<64>,
) {
    ...
    let _ = verify_mint_signature(&e, &admin_pubkey, &e.current_contract_address(),
        &user, period, &archetype, &data_hash, payload_version, &signature);
    ...
    let record = WrapRecord {
        timestamp: now, data_hash, archetype: archetype.clone(),
        period, fsm: WrapLifecycleFSM::new(WrapState::Active, now),
    };
    ...
    if current_count == 0 {
        storage_accounting::add_storage_bytes(&e, storage_accounting::estimate_wrapcount_bytes_new());
    }
    ...
    if period > current_latest {
        ...
        if was_missing {
            storage_accounting::add_storage_bytes(&e, storage_accounting::estimate_latest_bytes_new());
        }
    }
    let user_periods_key = DataKey::UserPeriods(user.clone());
    let mut periods: soroban_sdk::Vec<u64> = e.storage().persistent().get(&user_periods_key).unwrap_or(soroban_sdk::Vec::new(&e));
    if !periods.contains(period) {
        ...
        storage_accounting::add_storage_bytes(&e, storage_accounting::estimate_userperiods_bytes_new());
    }
    e.events().publish((symbol_short!("mint"), user, period), archetype);
}
pub(crate) fn transition_wrap_state(e: Env, user: Address, period: u64, next_state: WrapState) {
    crate::admin::require_not_paused(&e);
    user.require_auth();
    let wrap_key = DataKey::Wrap(user.clone(), period);
    let mut record: WrapRecord = e.storage().persistent().get(&wrap_key)
    ...
    e.storage().persistent().set(&wrap_key, &record);
    e.storage().persistent().extend_ttl(&wrap_key, TTL_ONE_YEAR, TTL_ONE_YEAR);
    e.events().publish((symbol_short!("trans"), user, period), next_state);
}